...
public class CreateProductwithMarketingGroupTest {
   ...
   //3.Select product from quick create drop down
	...
	//4. create a new product with product name-Sony
		...
		String productName=workbook.getSheet("Sheet1").getRow(3).getCell(2).getStringCellValue();
	...
	//	b)and select ""Marketing Group"" option from the dropdown
		...
		Select selectt=new Select(handlerGroupDropdown);
		 ...
		 WebElement selectedOption1 = selectt.getFirstSelectedOption();
			if(selectedOption1.getText().equals("Marketing Group")) {
				System.out.println("PASS::Marketing Group option is selected");
			}else {
				System.out.println("FAIL::Marketing Group option is not selected");
			}
