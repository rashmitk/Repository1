...
public class CreateProductWithTeamSellingTest {
   ...
   //3.Select product from quick create drop down
	...
	//4. create a new product with product name-Sony
		...
		String productName=workbook.getSheet("Sheet1").getRow(7).getCell(2).getStringCellValue();
	...
	//	b)and select ""Marketing Group"" option from the dropdown
		...
		Select selectt=new Select(handlerGroupDropdown);
		 ...
		 WebElement selectedOption1 = selectt.getFirstSelectedOption();
			if(selectedOption1.getText().equals("Team Selling")) {
				System.out.println("PASS::Team Selling option is selected");
			}else {
				System.out.println("FAIL::Team Selling option is not selected");
			}
